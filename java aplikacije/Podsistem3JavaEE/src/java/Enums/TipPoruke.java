...
public enum TipPoruke {
    ...
    public String toString() {
        String[] delovi = this.name().toLowerCase().split("_");
        StringBuilder sb = new StringBuilder(delovi[0]);
        for (int i = 1; i < delovi.length; i++) {
            sb.append(Character.toUpperCase(delovi[i].charAt(0)))
              .append(delovi[i].substring(1));
        }
        return sb.toString(); // npr. "kreirajMesto"
    }
}
